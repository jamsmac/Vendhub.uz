...
import type { CooperationRequest, PartnershipModel } from '@/lib/types'
...
export default function RequestsTab({ partnershipModels }: RequestsTabProps) {
  ...
  const tp = useTranslations('admin.partnership.requests')
  ...
  // Build model filter options from partnership_models
  const modelFilters = [
    { value: '', label: t('modelFilters.all') },
    ...partnershipModels.map((m) => ({ value: m.key, label: m.title })),
  ]
  ...
  // Map model key → title for display
  const modelLabels: Record<string, string> = {}
  for (const m of partnershipModels) {
    modelLabels[m.key] = m.title
  }
  ...
  const handleConfirmDelete = async () => {
    ...
    const { error } = await supabase.from('cooperation_requests').delete().eq('id', deleteTarget.id)
    if (error) {
      showToast(tp('deleteError'), 'error')
    } else {
      showToast(tp('deleted'), 'success')
      fetchRequests()
      window.dispatchEvent(new Event('cooperation-updated'))
    }
  ...
  const getModelLabel = (key: string) => modelLabels[key] ?? key
  ...
  return (
    <div className="space-y-4">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-espresso/10 bg-foam/50">
                ...
                <th className="text-left px-4 py-3 font-medium text-espresso/50">{t('table.phone')}</th>
                <th className="text-left px-4 py-3 font-medium text-espresso/50">{t('table.comment')}</th>
                <th className="text-center px-4 py-3 font-medium text-espresso/50">{t('table.status')}</th>
                <th className="text-left px-4 py-3 font-medium text-espresso/50">{t('table.date')}</th>
                <th className="w-10" />
              </tr>
            </thead>
            <tbody>
                  ...
                  <tr key={req.id} onClick={() => openDetail(req)}
                    ...
                    <td className="px-4 py-3">
                      <span className="inline-block px-2.5 py-0.5 rounded-full text-xs font-medium bg-espresso/5 text-espresso/60">
                        {getModelLabel(req.model)}
                      </span>
                    </td>
                    <td className="px-4 py-3 font-medium text-espresso">{req.name}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-espresso/50 max-w-xs truncate">{req.comment ?? '—'}</td>
                    <td className="px-4 py-3 text-center">
                      <select value={req.status} onClick={(e) => e.stopPropagation()}
                        ...
                        className={['text-xs font-medium rounded-full px-3 py-1 border-none outline-none cursor-pointer', statusColors[req.status] ?? ''].join(' ')}>
                        {(['new', 'read', 'processed'] as const).map((val) => (
                          <option key={val} value={val}>{t(`statusLabels.${val}`)}</option>
                        ))}
                      </select>
                    </td>
                    <td className="px-4 py-3 text-espresso/40 text-xs whitespace-nowrap">{formatDate(req.created_at)}</td>
                    <td className="px-4 py-3">
                      <button type="button"
                        onClick={(e) => { e.stopPropagation(); setDeleteTarget({ id: req.id, name: req.name }) }}
      ...
      <ConfirmDialog
        ...
        message={tp('deleteMessage', { name: deleteTarget?.name ?? '' })}
      ...
      {selectedRequest && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto"
          role="dialog" aria-modal="true" aria-labelledby="cooperation-detail-title"
          ...
          <div className="w-full max-w-lg bg-white rounded-2xl shadow-2xl my-8" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-espresso/10">
              <h2 id="cooperation-detail-title" className="text-lg font-bold text-espresso">{t('details')}</h2>
              <button type="button" onClick={() => setSelectedRequest(null)} aria-label={tc('close')}
            ...
            <div className="p-6 space-y-4">
              <div>
                <span className="text-xs font-medium text-espresso/40 uppercase tracking-wider">{t('table.model')}</span>
                <p className="mt-1">
                  <span className="inline-block px-2.5 py-0.5 rounded-full text-xs font-medium bg-espresso/5 text-espresso/60">
                    {getModelLabel(selectedRequest.model)}
                  </span>
                </p>
              </div>
              <div>
                <span className="text-xs font-medium text-espresso/40 uppercase tracking-wider">{t('table.name')}</span>
                <p className="mt-1 font-medium text-espresso">{selectedRequest.name}</p>
              </div>
              <div>
                <span className="text-xs font-medium text-espresso/40 uppercase tracking-wider">{t('table.phone')}</span>
                <p className="mt-1">
              ...
              <div>
                <span className="text-xs font-medium text-espresso/40 uppercase tracking-wider">{t('table.comment')}</span>
                <p className="mt-1 text-espresso/60">{selectedRequest.comment || '—'}</p>
              </div>
              <div>
                <span className="text-xs font-medium text-espresso/40 uppercase tracking-wider">{t('table.date')}</span>
                <p className="mt-1 text-sm text-espresso/60">{formatDate(selectedRequest.created_at)}</p>
              </div>
              <div>
                <span className="text-xs font-medium text-espresso/40 uppercase tracking-wider">{t('table.status')}</span>
                <div className="mt-1">
                  <select value={selectedRequest.status}
                    onChange={(e) => handleDetailStatusChange(e.target.value as CooperationRequest['status'])}
                    className={['text-sm font-medium rounded-full px-4 py-1.5 border-none outline-none cursor-pointer', statusColors[selectedRequest.status] ?? ''].join(' ')}>
                    {(['new', 'read', 'processed'] as const).map((val) => (
                      <option key={val} value={val}>{t(`statusLabels.${val}`)}</option>
                    ))}
                  </select>
              ...
              <div className="pt-2 border-t border-espresso/10">
                <label className="block text-xs font-medium text-espresso/40 uppercase tracking-wider mb-1.5">{t('adminNotes')}</label>
                <textarea value={notesValue} onChange={(e) => setNotesValue(e.target.value)}
                  placeholder={t('notesPlaceholder')} rows={3} className="admin-input w-full text-sm resize-y" />
